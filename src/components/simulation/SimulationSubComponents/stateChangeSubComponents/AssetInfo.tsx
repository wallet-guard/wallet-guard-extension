...
export const AssetInfo = (props: StateChangesComponentProps) => {
  ...
  const title = !isNftChangeType ?
    ...
    props.stateChange.tokenName ? add3Dots(props.stateChange.tokenName, 15) :
      props.stateChange.tokenID ? add3Dots(props.stateChange.tokenID, 15) :
        props.stateChange.name ? add3Dots(props.stateChange.name, 15) : 'Unknown';
  const titleLink = isNftChangeType ? props.stateChange.openSeaLink : props.stateChange.coinmarketcapLink;
...
function AssetTitle(props: AssetTitleProps) {
  const { title, locked, link, shouldShowSubtitle, openSeaVerified } = props;
  ...
  return (
    <div className={styles['row']}>
      ...
      {(!shouldShowSubtitle && openSeaVerified) && (
        <Tooltip
          hasArrow
